...
func TestXAIReasoningWithToolsOmitsInstructionsOnFollowup(t *testing.T) {
	...
	sessionID, err := service.GetReasoningStructuredOutputWithToolsForSession(context.Background(), nil, nil, schema, "grok-4-1-fast-reasoning", &maxTokens, messages, tools, toolHandlers, nil, nil, nil, nil, false, 4, &firstOutput)
	if err != nil {
	...
	if _, err := service.GetReasoningStructuredOutputWithToolsForSession(context.Background(), &sessionID, nil, schema, "grok-4-1-fast-reasoning", &maxTokens, messages, tools, toolHandlers, nil, nil, nil, nil, false, 4, &secondOutput); err != nil {
