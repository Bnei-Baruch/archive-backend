...
type GetSourceFilterValuesTool struct{}

func NewGetSourceFilterValuesTool() *GetSourceFilterValuesTool {
	return &GetSourceFilterValuesTool{}
}
...
Behavior:
- Returns the list of all valid source filter values, with English and Hebrew names, including the full hierarchy parents path (like book->part->chapter). The list is returned as a string with each line containing one source filter value in the following format: {id} en: {English name with parents path} he: {Hebrew name with parents path}. For example: "ALlyoveA en: Baal HaSulam => Prefaces => Introduction to the Book of Zohar he: בעל הסולם => הקדמות => הקדמה לספר הזוהר".`
}

func (t *GetSourceFilterValuesTool) Execute(ctx context.Context, arguments json.RawMessage) (string, error) {

	// TBD need to prepare cached generated from DB values
	const filtersList = `bs en: Baal HaSulam he: בעל הסולם
...
bb en: Bnei Baruch he: בני ברוך`
	...
	if err := validateEmptyToolArguments("get_source_filter_values", arguments); err != nil {
		return "", err
	}
	llm.LogIfDeb(ctx, "get_source_filter_values: returning cached filters list")
	return filtersList, nil
}

func validateEmptyToolArguments(toolName string, arguments json.RawMessage) error {
	if len(arguments) == 0 {
		return nil
	}
	if strings.TrimSpace(string(arguments)) == "" || strings.TrimSpace(string(arguments)) == "null" {
		return nil
	}

	values := map[string]interface{}{}
	if err := json.Unmarshal(arguments, &values); err != nil {
		return fmt.Errorf("%s: failed to parse arguments: %w", toolName, err)
	}
	if len(values) > 0 {
		return fmt.Errorf("%s: this tool does not accept arguments", toolName)
	}
	return nil
}
