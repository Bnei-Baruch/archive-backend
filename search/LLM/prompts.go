...
When searching a name of a specific Kabbalist author, prioritize relevant programs or lessons that are dedicated to that specific kabbalist (his role in Kabbalah). If not explicitly asked, no need to include the source items of his works, if they are not root items (main library items).
...
If the archive is not containing any data that can be relevant to the user query, you should return an empty list of results, set the no_results field to true, and set summary to null.
Set no_results=false in every other case, including uncertainty, incomplete investigation, tool issues, or weak but possible matches.
...
The term 'ten' is used to refer to a small group of students (usually 10 or more) that practice between themselves connection according to Kabbalistic principles. Basicaly, most of the students are part of a ten, and the ten is the main framework for practicing connection. Ten names are usually consisted of a letters that present a group location (city or country), language, or study framework (like MAK, Unity), and a number of the ten in that group. For example, 'PT 2' referes to ten 2 in Petah Tikva, Israel. The letter 'W' is usually added to the ten name to indicate that it is a women ten.
...
const ReasoningSearchRapidClassificationInstructionMask = `Classify archive search candidates for relevance to the user query. Also look at the previous_response queries to understand the context of the current query.
...
- highly_relevant: direct strong match to the user's request, compatible with both the desired type of content and the content itself. Set this value only on a high confidence that the candidate matches the user intention, otherwise set other value. If user looks for a kabbalist author by his name, and the candidate is reffered to just a single source items of his works, then it is not highly_relevant.
- relevant: useful match, but not the strongest. If the user query looks like a citation from a source but the matching result is not source but program, lesson or blog post then set as relevant but not highly_relevant. When the user looks for a kabbalist author by his name, and the candidate is a main book or root item in the library, then it is relevant (but not highly_relevant). But if the item is refered to some single article or excerpt of his works, this is not necessarily relevant.
...
func GenerateSystemMessageForReasoningSearch(tools []ReasoningTool, remainingIterations int, followupsRemaining int) string {
	return generateSystemMessageForSearchInstruction(
		fmt.Sprintf("%s\n%s", GeneralReasoningSearchBaseInstruction, ReasoningSearchClarificationInstruction),
		tools,
		remainingIterations,
		followupsRemaining,
	)
...
func GenerateSystemMessageForRapidReasoningSearch(tools []ReasoningTool, remainingIterations int, followupsRemaining int) string {
	return generateSystemMessageForSearchInstruction(
		GeneralReasoningSearchBaseInstruction,
		tools,
		remainingIterations,
		followupsRemaining,
	)
...
func generateSystemMessageForSearchInstruction(instruction string, tools []ReasoningTool, remainingIterations int, followupsRemaining int) string {
	...
	msg = fmt.Sprintf("%s\n\nOptimal number of results to return is %d.", msg, ReasoningSearchOptimalResultCount)
...
func GenerateSystemMessageForReasoningSearchPlanning(tools []ReasoningTool) string {
	...
	message := fmt.Sprintf(ReasoningSearchPlanningInstructionMask, instruction)
	toolUsage := buildReasoningSearchToolUsage(tools)
	if toolUsage == "" {
		return message
	}
